...
> **为什么 Rust 的 `&mut self` 和 JS/Java 的 `this` 不一样？**  
> Rust 要求你在函数签名里显式声明 `self` 的访问方式（`self` / `&self` / `&mut self`），而 JS/Java 的 `this` 是隐式且总是可读写的。详见 [self vs this 对比笔记](../notes/self-vs-this.md)。
...
```rust
    fn draw(&mut self, frame: &mut Frame, area: Rect) -> color_eyre::Result<()> {
        ...
        // 4. actaully render the widgets in the view

...
Rust 的 `self` 设计之所以和 JS/Java 不同，**根源是所有权系统**。

- JS/Java：`this` 只是一个指向实例的引用，实例由 GC 管理，方法调用不会"消耗"实例
- Rust：`self` 参数决定了**调用后实例是否还存在** —— 被 move 了就没了，被借用就还活着

所有权让 `&self` / `&mut self` / `self` 的区分有了实际意义。详见 [所有权笔记](./ownership.md)。
